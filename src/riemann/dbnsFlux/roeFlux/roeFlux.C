...
{   
    const scalar gm1  = gamma - 1;
    // left & right state
    scalar temp = 1.0/rho_L;
    const vector U_L = rhoU_L*temp;
    const scalar p_L = (rhoE_L - 0.5*rho_L*magSqr(U_L)) * gm1;
    const scalar H_L = (rhoE_L + p_L)*temp;
    temp = 1.0/rho_R;
    const vector U_R = rhoU_R*temp;
    const scalar p_R = (rhoE_R - 0.5*rho_R*magSqr(U_R)) * gm1;
    const scalar H_R = (rhoE_R + p_R)*temp;

    // left and right V*n
    const scalar Vn_L = U_L & normal;
    const scalar Vn_R = U_R & normal;
    const scalar p_A = 0.5 * (p_L + p_R);
    rhoFlux  = 0.5 * (Vn_L*rho_L  + Vn_R*rho_R);
    rhoUFlux = 0.5 * (Vn_L*rhoU_L + Vn_R*rhoU_R) + p_A*normal;
    rhoEFlux = 0.5 * (Vn_L*(rhoE_L+p_L) + Vn_R*(rhoE_R+p_R));
    ...
    const scalar c2_A  = gm1*(H_A-U2_A);
